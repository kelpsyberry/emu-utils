...
impl<T> Sender<T> {
    ...
    pub fn current_i(&self) -> u8 {
        self.i
    }

    pub fn finish(&mut self) {
        self.i = self.buffers.next.swap(self.i | 4, Ordering::Release) & 3;
    }
}
    ...
    pub fn get(&mut self) -> Result<&T, &T> {
        unsafe {
            if self.buffers.next.load(Ordering::Relaxed) & 4 != 0 {
                self.i = self.buffers.next.swap(self.i, Ordering::Acquire) & 3;
...
pub unsafe fn init_zeroed<T>() -> (Sender<T>, Receiver<T>) {
    let buffers = Arc::<Buffers<T>>::new_zeroed().assume_init();
    buffers.next.store(1, Ordering::Relaxed);
    (
        Sender {
            buffers: Arc::clone(&buffers),
            i: 0,
