...
fn emit_bitfield(input: TokenStream, emit_debug_impl: bool) -> TokenStream {
    let Struct {
        outer_attrs,
        vis,
        ident,
        ...
        |Field {
            ...
            let getter = if !matches!(&get_ty, AccessorKind::Disabled) {
                let (calc_get_result, get_output_ty) = match get_ty {
                    ...
                    AccessorKind::UnsafeConv(get_ty) => {
                        (
                            quote! { unsafe {
                                <#get_ty as ::emu_utils::bitfield::UnsafeFrom<#ty>>::from(
                                    raw_result,
                                )
                            } },
                            quote! { #get_ty },
                ...
                let get_value = if let Some(end) = &end {
                    quote_spanned! {
                        ident.span() =>
                        ::emu_utils::static_assertions::const_assert!(
                            #end > #start
                        );
                        ::emu_utils::static_assertions::const_assert!(
                        ...
                        let raw_result = <#storage_ty as ::emu_utils::bitfield::BitRange<#ty>>
                            ::bit_range::<#start, #end>(self.0);
                    }
                } else {
                    quote_spanned! {
                        ident.span() =>
                        ::emu_utils::static_assertions::const_assert!(#start < #storage_ty_bits);
                        let raw_result = <#storage_ty as ::emu_utils::bitfield::Bit>
                            ::bit::<#start>(self.0);
                    }
                };
                quote! {
            ...
            let setters = if !matches!(&set_ty, AccessorKind::Disabled) {
                ...
                let with_value = if let Some(end) = &end {
                    quote_spanned! {
                        ident.span() =>
                        <#storage_ty as ::emu_utils::bitfield::BitRange<#ty>>
                            ::set_bit_range::<#start, #end>(self.0, #calc_set_with_value)
                    }
                } else {
                    quote_spanned! {
                        ident.span() =>
                        <#storage_ty as ::emu_utils::bitfield::Bit>::set_bit::<#start>(
                            self.0,
                            #calc_set_with_value,
                        )
                    }
    ...
    let impl_debug = if emit_debug_impl {
